/* TODO */
/* Make sure your secure chat application handles loss of the control messages specific to the secure chat application appropriately with timers and retries. */
/* support session resumption using session tickets */

/* Alice receives chat_START_SSL_NOT_SUPPORTED  */
#include <cstdlib>
#include <cstring>
#include <netdb.h>
...
int my_new_session_cb(SSL *ssl, SSL_SESSION *session) {
    unsigned int session_id_length;
    const unsigned char *session_id = SSL_SESSION_get_id(session, &session_id_length);

	return 0;
    // Store session ID and session data in a cache or perform any necessary actions
}
...
void clientMode(char *hostname) {
    ...
    char *not_ssl_res_ptr = strstr(start_ssl_res.buf, "chat_START_SSL_NOT_SUPPORTED");
    if (not_ssl_res_ptr == NULL) {
        error("Error for chat_START_SSL_NOT_SUPPORTED");
    }
...
/* Create a SSL_CTX structure */
	...
	SSL_CTX_set_mode(ctx,SSL_MODE_AUTO_RETRY);
    /* SSL_CTX_set_session_cache_mode(ctx,SSL_SESS_CACHE_OFF); */
	/* added the below line for resumption */
    SSL_CTX_set_session_cache_mode(ctx,SSL_SESS_CACHE_CLIENT);
	SSL_CTX_sess_set_new_cb(ctx, my_new_session_cb);
